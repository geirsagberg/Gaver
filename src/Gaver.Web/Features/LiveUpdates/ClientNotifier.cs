...
public class ClientNotifier(IHubContext<ListHub, IListHubClient> hub) : IClientNotifier {
    public Task RefreshListAsync(int wishListId, int? excludeUserId = null) => hub.RefreshDataAsync(wishListId, excludeUserId);

    public Task MessageAdded(int wishListId, ChatMessageDto chatMessage) {
        return chatMessage.User == null
            ? Task.CompletedTask
            : hub.Clients.GroupExcept(ListHub.GetGroup(wishListId),
